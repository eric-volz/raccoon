...
from ref_py.nist_kat_drbg import NIST_KAT_DRBG
from ref_py.mask_random import MaskRandom
from ref_py.polyr import *
...
class Raccoon:
    ...
    def sign_mu_fault_attack_safe(self, lst_msk: tuple, lst_mu: list, e: int, inject_error: callable = None) -> tuple:
        ...
        inject_error(lst_seed)  # 0
        inject_error(lst_t)  # 1
        inject_error(lst_ms_ntt)  # 2

        #   --- 2.  mu := H( H(vk) || msg )                     [ caller ]

        #   --- 3.  A := ExpandA(seed)
        lst_A_ntt = [mat_ntt(self._expand_a(seed)) for seed in lst_seed]
        inject_error(lst_A_ntt)  # 3
